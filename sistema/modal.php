...
// Buscar productos realizado por SCRIPT
if ($_POST['action'] == 'searchCliente_v') {
  if (!empty($_POST['codigo_producto'])) {
    $codigo_producto = $_POST['codigo_producto'];
   

    $query = mysqli_query($conexion, "SELECT * FROM productos WHERE codigo_producto LIKE $codigo_producto");
    mysqli_close($conexion);
    $result = mysqli_num_rows($query);
    $data = '';
    if ($result > 0) {
...
// Buscar productos en detalles ventas y tomar la cantidad para comparar con el stock temporalmente. realizado por SCRIPT
if ($_POST['action'] == 'searchCliente_v5') {
  if (!empty($_POST['codigo_producto'])) {
    $codigo_producto = $_POST['codigo_producto'];
   

    $query = mysqli_query($conexion, "SELECT * FROM detalle_ventas WHERE codigo_producto LIKE $codigo_producto and estado_detalle=0");
...
// Buscar CLIENTE realizado por SCRIPT
if ($_POST['action'] == 'searchCliente_y') {
  if (!empty($_POST['nit_cliente'])) {
    $nit_cliente = $_POST['nit_cliente'];
   

    $query = mysqli_query($conexion, "SELECT * FROM clientes WHERE nit_cliente LIKE '$nit_cliente'");
    mysqli_close($conexion);
    $result = mysqli_num_rows($query);
    $data = '';
    if ($result > 0) {
...
// Buscar productos realizado por SCRIPT
if ($_POST['action'] == 'searchCliente_p') {
  if (!empty($_POST['dpi_personal'])) {
    $dpi_personal = $_POST['dpi_personal'];
   

    $query = mysqli_query($conexion, "SELECT * FROM  mi_personal WHERE dpi_personal LIKE $dpi_personal");
    mysqli_close($conexion);
    $result = mysqli_num_rows($query);
    $data = '';
    if ($result > 0) {
...
if ($_POST['action'] == 'searchCliente_p2') {
  if (!empty($_POST['dpi_personal2'])) {
    $dpi_personal2 = $_POST['dpi_personal2'];
   

    $query = mysqli_query($conexion, "SELECT * FROM  mi_personal WHERE dpi_personal LIKE $dpi_personal2");
    mysqli_close($conexion);
    $result = mysqli_num_rows($query);
    $data = '';
    if ($result > 0) {
...
// Buscar id de la venta para el express realizado por SCRIPT
if ($_POST['action'] == 'searchCliente_c') {
  if (!empty($_POST['codigo_venta'])) {
    $codigo_venta = $_POST['codigo_venta'];
   

    $query = mysqli_query($conexion, "SELECT * FROM ventas_creadas WHERE codigo_venta LIKE '$codigo_venta'");
    mysqli_close($conexion);
    $result = mysqli_num_rows($query);
    $data = '';
    if ($result > 0) {
