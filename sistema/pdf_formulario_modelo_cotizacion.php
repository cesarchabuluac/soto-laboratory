...
$query = mysqli_query($conexion,"SELECT p.id_paciente,p.dpi_paciente,p.nombre_paciente,p.edad_paciente,p.id_genero,p.telefono_paciente,p.correo_paciente,p.id_usuario, p.direccion_paciente,p.fecha_nacimiento_paciente, u.id_usuario,u.usuario,g.id_genero,g.nombre_genero,exc.id_paciente,exc.id_examen_creado,exc.fecha_creado,exc.hora_creado,exc.id_usuario_tecnico FROM pacientes p INNER JOIN usuario u ON p.id_usuario=u.id_usuario INNER JOIN genero g ON g.id_genero=p.id_genero INNER JOIN examenes_creados exc ON exc.id_paciente=p.id_paciente WHERE exc.id_examen_creado=$id_examen_creado"); 
...
foreach ($pacientes as $paciente_ecnontrado) {
 
  $fecha_creado=$paciente_ecnontrado['fecha_creado'];
   $hora_creado=$paciente_ecnontrado['hora_creado'];
$edad_paciente=$paciente_ecnontrado['edad_paciente'];
 $nombre_paciente=$paciente_ecnontrado['nombre_paciente'];
 $dpi_paciente=$paciente_ecnontrado['dpi_paciente'];
 $fecha_nacimiento_paciente=$paciente_ecnontrado['fecha_nacimiento_paciente'];
 $id_examen_creado=$paciente_ecnontrado['id_examen_creado'];
  $usuario=$paciente_ecnontrado['usuario'];
  $id_usuario_tecnico=$paciente_ecnontrado['id_usuario_tecnico'];
...
$query = mysqli_query($conexion,"SELECT d.id_detalle_examen,d.id_examen_creado,d.resultado,d.id_area,d.id_paciente,d.descripcion,d.estado_detalle,d.precio_examen,e.id_examen,e.nombre_examen,e.valor_referencia,e.nombre_nomenclatura,e.subarea FROM detalle_examenes d INNER JOIN examenes e ON d.id_examen=e.id_examen  WHERE d.id_examen_creado=$id_examen_creado"); 
...
<th bgcolor="  #0c6de8  " style="font-size: 12px;font-family: sans-serif;text-align: center;"><font color="white">Precio</th>
...
<tbody style="text-align:center;" >
 ...
 foreach ($examenes_agregados as $agregado) {
  ...
  ?>
<tr>
  <td style="font-size: 13px;font-family: sans-serif;text-align: center;"><?php echo $fila=$fila+1;  ?></td>
      <td style="font-size: 13px;font-family: sans-serif;text-align: center;"><?php echo $agregado['nombre_examen'] ?></td>

  
      <td style="font-size: 13px;font-family: sans-serif;text-align: center;padding: 4px;">Q <?php echo $precio_examen=$agregado['precio_examen']?>
        

      </td>

      <?php echo 
      $precio_total=$precio_total+$precio_examen;?>
...
<tr><td></td><td></td><td style="font-size:17px">= Q <?php echo number_format($precio_total,2) ?></td><tr>
